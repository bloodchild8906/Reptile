...
public static class SpellingExtensions
{
    ...
    {
        return spellingVariants! switch
        {
    ...
    private static IEnumerable<Tuple<string, string>> Splits(HashSet<string> set, string word) => word
        .Select((_, i) => new Tuple<string, string>(word[..i], word.Substring(i, word.Length - i)))
        .ToList();
    ...
    private static IEnumerable<string> Replaces(IEnumerable<Tuple<string, string>> splits) =>
        Alphabet.SelectMany(x => Replaces(splits, x));
    ...
    private static IEnumerable<string> Inserts(IEnumerable<Tuple<string, string>> splits) =>
        Alphabet.SelectMany(x => Inserts(splits, x));
